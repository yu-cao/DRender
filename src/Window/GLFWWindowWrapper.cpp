...
{
	InputManager::Action inputAction = InputManager::Action::NONE;
	...
	switch (glfwAction) {
		...
		case -1: break;
		default:
			Logger::LogError("Unhandled glfw action passed to GLFWActionToInputManagerAction in GLFWWIndowWrapper: " + std::to_string(glfwAction));
	}
...
{
	InputManager::KeyCode inputKey = InputManager::KeyCode::NONE;
...
int GLFWModsToInputManagerMods(int glfwMods)
{
	...
	if (glfwMods & GLFW_MOD_SUPER) inputMods |= (int)InputManager::Mod::CAPS;
...
{
	InputManager::MouseButton inputMouseButton = InputManager::MouseButton::NONE;
...
void GLFWKeyCallback(GLFWwindow* glfwWindow, int key, int scancode, int action, int mods)
{
	...
	const auto inputAction = GLFWActionToInputManagerAction(action);
	const auto inputKey = GLFWKeyToInputManagerKey(key);
...
void GLFWMouseButtonCallback(GLFWwindow* glfwWindow, int button, int action, int mods)
{
	...
	const auto inputAction = GLFWActionToInputManagerAction(action);
	const int inputMods = GLFWModsToInputManagerMods(mods);
	const auto mouseButton = GLFWButtonToInputManagerMouseButton(button);
